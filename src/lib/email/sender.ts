...
const FROM = process.env.EMAIL_FROM || 'Nexus <noreply@nexus.app>'
...
export async function sendVerificationEmail(email: string, code: string, name?: string) {
  ...
  `
  await transporter.sendMail({ from: FROM, to: email, subject: 'Verify your Nexus account', html: baseTemplate(content) })
}
...
export async function sendPasswordResetEmail(email: string, code: string) {
  ...
  `
  await transporter.sendMail({ from: FROM, to: email, subject: 'Reset your Nexus password', html: baseTemplate(content) })
}
...
export async function sendWelcomeEmail(email: string, name: string) {
  ...
  `
  await transporter.sendMail({ from: FROM, to: email, subject: 'Welcome to Nexus! 🚀', html: baseTemplate(content) })
}
